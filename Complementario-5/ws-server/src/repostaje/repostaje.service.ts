...
export class RepostajeService {
  ...
  async create(createRepostajeDto: CreateRepostajeDto): Promise<any>  {
    try {
      const repostaje =  this.repostajeRepository.create(createRepostajeDto);
  ...
  async update(REPOSTAJE_ID: number, updateRepostajeDto: UpdateRepostajeDto): Promise<Repostaje> {
  ...
  async remove(REPOSTAJE_ID: number) {
    ...
    // return `This action removes a #${id} repostaje`;
  }
  prueba():String[]{
    return ['uno','dos','tres'];
  }
